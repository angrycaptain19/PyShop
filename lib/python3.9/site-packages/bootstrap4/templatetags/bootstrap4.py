...
def bootstrap_css():
    ...
    **Example**::
    ...
    return mark_safe("".join(rendered_urls))
...
class ButtonsNode(template.Node):
    ...
    def render(self, context):
        output_kwargs = {
            key: handle_var(self.kwargs[key], context) for key in self.kwargs
        }
...
def get_pagination_context(
    ...
    half_page_num = max(half_page_num, 0)
    first_page = current_page - half_page_num
    first_page = max(first_page, 1)
    if first_page > 1:
        pages_back = first_page - half_page_num
        pages_back = max(pages_back, 1)
    else:
        pages_back = None
    last_page = first_page + pages_to_show - 1
    if pages_back is None:
        last_page += 1
    last_page = min(last_page, num_pages)
    if last_page < num_pages:
        pages_forward = last_page + half_page_num
        pages_forward = min(pages_forward, num_pages)
    else:
        pages_forward = None
        if first_page > 1:
            first_page -= 1
        if pages_back is not None and pages_back > 1:
            ...
            pages_back = None
    pages_shown = [i for i in range(first_page, last_page + 1)]
    # parse the url
